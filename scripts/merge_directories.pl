...
sub compare {
    ...
    if( -e $target_path ) {
        ...
        if($timestamp_target == $timestamp_source) {
            ...
            my ($checksum_source) = `sha1sum -b '$File::Find::name'` =~ m|^(\S+)|;
            my ($checksum_target) = `sha1sum -b '$target_path'`      =~ m|^(\S+)|;
        ...
        # time stamps differ but contents are the same?
        # this means that very probably the lower timestamp is correcter
        # than the higher one. The lower will be applied to the target,
        # the source file will be removed.
        else {
            my ($checksum_source) = fexec('sha1sum', '-b', $File::Find::name) =~ m|^(\S+)|;
            my ($checksum_target) = fexec('sha1sum', '-b', $target_path)      =~ m|^(\S+)|;
            
            # file contents identical?
            if ($checksum_target eq $checksum_source) {
                
                # which file has the lower date?
                my $lower_timestamp = $timestamp_target;
                $lower_timestamp = $timestamp_source if ($timestamp_source < $timestamp_target);

                say "timestamp $lower_timestamp -> $target_path";
                utime(time, $lower_timestamp, $target_path);
...
# fexec -- fork and execute an external program,
#          capture its output
#
# If the output of an external program is needed,
# the most secure way to get it is not via Perl's 
# `backticks`, but to fork a child and read its
# output. Like this, shell escaping of weird file
# names is avoided. File that are passed to
# external programs now can contain any kind of
# characters, from quotes to dollar signs to
